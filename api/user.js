...
export default async function handler(req, res) {
  ...
  // Coins trust the client (no real economy yet; skin purchases must be able
  // to decrement). Cap at a sane ceiling to bound any obvious tampering.
  if (clientCoins !== null) {
    patch.coins = Math.min(clientCoins, 10_000_000);
  }
  // High score is monotonically increasing — take the max server-side so
  // stale/spoofed lower values can't overwrite a legit record.
  if (clientHigh !== null) {
